...
class EventParticipantDataTable extends BaseDataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('allowed_seat', function ($row) {
                if (!is_null($row->no_of_seats_filled_start) && !is_null($row->no_of_seats_filled_end)) {
                    return implode(', ', range($row->no_of_seats_filled_start, $row->no_of_seats_filled_end));
                }
                return 'N/A';
            })
            ->addColumn('action', function ($row) use ($EventId) {
                $editRoute = $EventId && $EventId !== 'all'
                    ? route('event.event-participant.edit', [$row->id])
                    : route('event-participant.edit', [$row->id]);
                ...
                return '<div class="task_view">
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right">
                                    ...
                                    <a class="dropdown-item delete-table-row-event-participant" href="javascript:;"
                                       data-id="' . $row->id . '">
    ...
    public function query(EventParticipant $model): QueryBuilder
    {
        ...
        $checkinpointid = $request->checkinpointid;
// dd($checkinpointid);
        ...
        $query = $model->newQuery()
            ->select(
                ...
                'event_registrations.name as event_registration_name'
            )
        ...
        if ($request->checkinpointid != 'all' && $request->checkinpointid != '') {
            $query->where('event_participants.checkin_point_id', $request->checkinpointid);
        }
        ...
        if (!empty($searchText)) {
            $query->where(function ($q) use ($searchText) {
                $q->where('event_participants.event_registration_id', 'like', "%$searchText%")
                    ->orWhere('event_participants.checkin_time', 'like', "%$searchText%")
    ...
    public function getColumns(): array
    {
        return [
            ...
            'event_registration_name' => ['data' => 'event_registration_name', 'name' => 'event_registration_name', 'title' => 'Student Name'],
            'event_name' => ['data' => 'event_name', 'name' => 'event_name', 'title' => 'Event'],
            'checkin_time' => ['data' => 'checkin_time', 'name' => 'checkin_time', 'title' => 'checkin_time'],
