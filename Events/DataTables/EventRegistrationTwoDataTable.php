...
class EventRegistrationTwoDataTable extends BaseDataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('panchayath_name', fn($row) => optional($row->panchayath)->name ?? 'N/A')

            // Add District Name
            ->addColumn('district_name', fn($row) => optional($row->district)->name ?? 'N/A')
            ...
            ->editColumn('whatsapp', fn($row) => '+' . $row->country_wtsap_phonecode . ' ' . $row->whatsapp)
            ->addIndexColumn()
            ->smart(false)
    ...
    public function query(EventRegistration $model): QueryBuilder
    {
        ...
        //     $query->whereBetween('event_registrations.created_at', [$startDate, $endDate]);
        // }
        if ($request->districtid != 'all' && $request->districtid != '') {
            $query->where('event_registrations.district_id', $request->districtid);
        }
        if ($request->panchayatid != 'all' && $request->panchayatid != '') {
            $query->where('event_registrations.panchayat_id', $request->districtid);
        }
        if ($request->gender != 'all' && $request->gender != '') {
            $query->where('event_registrations.sex', $request->gender);
        }
        // Apply search filter
    ...
    public function getColumns(): array
    {
        return [
            ...
            'district_name' => ['data' => 'district_name', 'name' => 'district_name', 'title' => 'District'],
            'panchayath_name' => ['data' => 'panchayath_name', 'name' => 'panchayath_name', 'title' => 'Panchayat'],
            'mobile' => ['data' => 'mobile', 'name' => 'mobile', 'title' => 'Mobile'],
            'whatsapp' => ['data' => 'whatsapp', 'name' => 'whatsapp', 'title' => 'Whatsapp'],
            'sex' => ['data' => 'sex', 'name' => 'sex', 'title' => 'Gender'],
            'pincode' => ['data' => 'pincode', 'name' => 'pincode', 'title' => 'Pincode'],
            'kids' => ['data' => 'kids_under_12', 'name' => 'kids_under_12', 'title' => 'Kids Under 12'],
            'i_agree' => ['data' => 'whatsapp_group_permission', 'name' => 'whatsapp_group_permission', 'title' => 'I Agree'],
