...
class DwcDepartureDataTable extends BaseDataTable
{
    ...
    public function query(DwcFlightTicket $model)
    {
        ...
        if ($request->searchText != '') {
            ...
            $query->where(function ($query) use ($request) {
                ...
                $query->where('dwc_flight_tickets.departure_date', 'like', $search)
                    ->orWhere('dwc_flight_tickets.departure_time', 'like', $search)
        ...
        if ($startDate && $endDate) {
            $query->whereBetween('dwc_flight_tickets.departure_date', [$startDate, $endDate]);
        }

        if ($startTime && $endTime) {
            $query->whereBetween('dwc_flight_tickets.departure_time', [$startTime, $endTime]);
        }
        if ($AirportId != 0 && $AirportId != null && $AirportId != 'all') {
            $query->where('flight_from', '=', $AirportId);
        }
        $query->whereIn('flight_from', [1517,1515,4822,1520,1518,1519,3708]);
        return $query;
    }
    ...
    {
        $dataTable = $this->setBuilder('dwc-departure-table', 2)
            ->parameters([
                'initComplete' => 'function () {
                    window.LaravelDataTables["dwc-departure-table"].buttons().container()
    ...
    protected function getColumns()
    {
        $data = [
            ...
            __('app.departure_date') => ['data' => 'departure_date', 'name' => 'departure_date', 'title' => __('app.departure_date')],
            __('app.departure_time') => ['data' => 'departure_time', 'name' => 'departure_time', 'title' => __('app.departure_time')],
            __('app.arrival_date') => ['data' => 'arrival_date', 'name' => 'arrival_date', 'visible' => false, 'title' => __('app.arrival_date')],
            __('app.arrival_time') => ['data' => 'arrival_time', 'name' => 'arrival_time', 'visible' => false, 'title' => __('app.arrival_time')],
