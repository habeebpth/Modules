...
use Modules\DWC\DataTables\DwcHotelsDataTable;
...
class DwcHotelsController extends AccountBaseController
{
    public function __construct()
    {
        parent::__construct();
        $this->pageTitle = 'app.Hotels';
    ...
    {
        $request->validate([
            'hotel_id' => 'required|integer|exists:dwc_hotels,id',
        ...
        return Reply::successWithData(__('messages.recordSaved'), [ 'roomType' => $roomType->id]);
    }
    public function roomtypeEdit($id)
    {
        $roomtype = DwcHotelRoomType::findOrFail($id);
        return view('dwc::hotel.ajax.roomtype-edit', compact('roomtype'));
    }


    public function index(DwcHotelsDataTable $dataTable)
    {
        if (!request()->ajax()) {
            $this->hotels = DwcHotel::all();
            // $this->skills = Skill::all();
        }

        // dd($this->data);

        return $dataTable->render('dwc::hotel.index', $this->data);
    }
    ...
    public function create()
    {
        ...
        $this->view = 'dwc::hotel.ajax.create';
