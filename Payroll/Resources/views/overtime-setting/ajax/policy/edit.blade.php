    ...
    <div class="portlet-body">
        <x-form id="updateOvertimePolicy" method="POST" class="ajax-form">
            ...
            <div class="form-body">
                <div class="row">
                    <div class="col-lg-12">
                        <x-forms.text fieldId="name" :fieldLabel="__('app.name')" :fieldValue="$policy->name" fieldName="name"
                            fieldRequired="true">
                        </x-forms.text>
                    </div>
                    {{-- <div class="col-lg-12">
                        <div class="form-group">
                            ...
                            <select class="form-control select-picker" name="pay_code" id="pay_code"
                                ...
                                @foreach ($payCodes as $payCode)
                                    <option value="{{ $payCode->id }}"
                                        @if ($policy->pay_code_id == $payCode->id) selected @endif>{{ $payCode->name }}
                                        ({{ $payCode->code }})
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <tr>
                                <td>
                                    <x-forms.checkbox :fieldLabel="__('payroll::modules.payroll.policyWorkingDays')" fieldName="working_days" fieldValue="yes"
                                        ...
                                        onchange="toggleDropdown('working_days', 'pay_code_working')" />
                                </td>
                                <td>
                                    <div class="col-lg-12">
                                        <div class="form-group">
                                            <select class="form-control select-picker" name="pay_code_working"
                                                id="pay_code_working" data-live-search="true"
                                                {{ !$policy || !$policy->working_days ? 'disabled' : '' }}>
                                                @foreach ($payCodes as $payCode)
                                                    <option value="{{ $payCode->id }}"
                                                        {{ $policy && $policy->pay_code_working == $payCode->id ? 'selected' : '' }}>
                                                        {{ $payCode->name }} ({{ $payCode->code }})
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                            ...
                            <tr>
                                <td>
                                    <x-forms.checkbox :fieldLabel="__('payroll::modules.payroll.policyWeekOffDays')" fieldName="week_end" fieldValue="yes"
                                        ...
                                        onchange="toggleDropdown('week_end', 'pay_code_weekend')" />
                                </td>
                                <td>
                                    <div class="col-lg-12">
                                        <div class="form-group">
                                            <select class="form-control select-picker" name="pay_code_weekend"
                                                id="pay_code_weekend" data-live-search="true"
                                                {{ !$policy || !$policy->week_end ? 'disabled' : '' }}>
                                                @foreach ($payCodes as $payCode)
                                                    <option value="{{ $payCode->id }}"
                                                        {{ $policy && $policy->pay_code_weekend == $payCode->id ? 'selected' : '' }}>
                                                        {{ $payCode->name }} ({{ $payCode->code }})
                                                    </option>
                                                @endforeach
                                            </select>
                            ...
                            <tr>
                                <td>
                                    <x-forms.checkbox :fieldLabel="__('payroll::modules.payroll.policyHolidayDays')" fieldName="holiday" fieldValue="yes"
                                        ...
                                        onchange="toggleDropdown('holiday', 'pay_code_holiday')" />
                                </td>
                                <td>
                                    <div class="col-lg-12">
                                        <div class="form-group">
                                            <select class="form-control select-picker" name="pay_code_holiday"
                                                id="pay_code_holiday" data-live-search="true"
                                                {{ !$policy || !$policy->holiday ? 'disabled' : '' }}>
                                                @foreach ($payCodes as $payCode)
                                                    <option value="{{ $payCode->id }}"
                                                        {{ $policy && $policy->pay_code_holiday == $payCode->id ? 'selected' : '' }}>
                                                        {{ $payCode->name }} ({{ $payCode->code }})
                                                    </option>
                                                @endforeach
                                            </select>
                    ...
                    <div class="col-lg-12 my-2 d-flex">
                        ...
                        <div class="input-radio-button mr-2 ml-2">
                            <select class="form-control" name="request_before_days" style="width:53px;">
                                @for ($i = 1; $i <= 31; $i++)
                                    <option value="{{ $i }}"
                                        {{ $policy?->request_before_days == $i ? 'selected' : '' }}>
                                        {{ $i }}</option>
                                @endfor
                            </select>
                        </div>
                        ...
                        <div class="form-group my-3">
                            ...
                            <select name="allow_roles[]" id="allow_roles" multiple class="form-control select-picker"
                                ...
                                @foreach ($roles as $role)
                                    <option @if (in_array($role->id, $storedRoles)) selected @endif
...
<script>
    ...
    $('#pay_code_working').selectpicker();
    $('#pay_code_holiday').selectpicker();

    $('#pay_code_weekend').selectpicker();
