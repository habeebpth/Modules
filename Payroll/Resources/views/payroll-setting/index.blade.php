                ...
                <div class="s-b-n-header" id="tabs">
                    <nav class="tabs px-4 border-bottom-grey">
                        <div class="nav" id="nav-tab" role="tablist">
                            ...
                            <a class="nav-item nav-link f-15 months-days"
                            href="{{ route('payroll.payroll_settings') }}?tab=months-days"
                            role="tab" aria-controls="nav-paymentMethods" aria-selected="true">
                             @lang('payroll::modules.payroll.monthsDays')
                         </a>
                         <a class="nav-item nav-link f-15 expense-type"
                            href="{{ route('payroll.payroll_settings') }}?tab=expense-type"
                            role="tab" aria-controls="nav-paymentMethods" aria-selected="true">
                             @lang('payroll::modules.payroll.ExpenseTypes')
                         </a>
                        </div>
                    </nav>
                </div>
            ...
            <x-slot name="buttons">
                <div class="row">
                    <div class="col-md-12 mb-2">
                        ...
                        <x-forms.button-primary icon="plus" id="add-e-type" class="expense-type-btn mb-2 d-none actionBtn">
                            @lang('payroll::modules.payroll.addNewExpenseType')
                        </x-forms.button-primary>
...
@push('scripts')
    <script>
        ...
        $('body').on('click', '#add-e-type', function () {
            var url = "{{ route('expense-type-settings.create') }}";
            $(MODAL_LG + ' ' + MODAL_HEADING).html('...');
            $.ajaxModal(MODAL_LG, url);
        });
        $('body').on('click', '.edit-expensetype', function () {
            var expensetypeId = $(this).data('expensetype-id');
            var url = "{{ route('expense-type-settings.edit', ':id') }}";
            url = url.replace(':id', expensetypeId);

            $(MODAL_LG + ' ' + MODAL_HEADING).html('...');
            $.ajaxModal(MODAL_LG, url);
        });
        $('body').on('click', '.delete-expensetype', function () {
            let obj = $(this).closest('tr');
            var id = $(this).data('expensetype-id');
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    var url = "{{ route('expense-type-settings.destroy', ':id') }}";
