...
class OvertimePolicyController extends AccountBaseController
{
    ...
    public function store(PolicyStoreRequest $request)
    {
        ...
        if ($request->has('working_days')) {
            $overtimePolicy->working_days = 1;
            $overtimePolicy->pay_code_working_days = $request->pay_code_working_days;
        }

        if ($request->has('week_end')) {
            $overtimePolicy->week_end = 1;
            $overtimePolicy->pay_code_week_end = $request->pay_code_week_end;
        }

        if ($request->has('holiday')) {
            $overtimePolicy->holiday = 1;
            $overtimePolicy->pay_code_holiday = $request->pay_code_holiday;
        }
    ...
    public function update(PolicyUpdateRequest $request, $id)
    {
        ...
        // Update working_days and pay_code_working_days only if checked
        if ($request->has('working_days')) {
            $overtimePolicy->working_days = 1;
            $overtimePolicy->pay_code_working_days = $request->pay_code_working_days;
        } else {
            $overtimePolicy->working_days = 0;
            $overtimePolicy->pay_code_working_days = null;
        }

        // Update week_end and pay_code_week_end only if checked
        if ($request->has('week_end')) {
            $overtimePolicy->week_end = 1;
            $overtimePolicy->pay_code_week_end = $request->pay_code_week_end;
        } else {
            $overtimePolicy->week_end = 0;
            $overtimePolicy->pay_code_week_end = null;
        }

        // Update holiday and pay_code_holiday only if checked
        if ($request->has('holiday')) {
            $overtimePolicy->holiday = 1;
            $overtimePolicy->pay_code_holiday = $request->pay_code_holiday;
        } else {
            $overtimePolicy->holiday = 0;
            $overtimePolicy->pay_code_holiday = null;
        }
        $overtimePolicy->save();
