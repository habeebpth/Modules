...
use Modules\Payroll\Imports\EmployeeSalaryImports;
use App\Models\Company; // or wherever your Company model is
...
use Modules\Payroll\Http\Requests\Employeesalary\ImportRequest;
use Modules\Payroll\Http\Requests\Employeesalary\ImportProcessRequest;
use Modules\Payroll\Jobs\ImportEmployeesalaryJob;
use Modules\Payroll\Imports\EmployeesalaryImport;
...
class EmployeeMonthlySalaryController extends AccountBaseController
{
    ...
    {
        $this->pageTitle = __('app.importExcel') . ' ' . __('payroll::app.menu.employeeSalary');
    ...
    // {
    //     $this->importFileProcess($request, EmployeesalaryImport::class);
    ...
    public function import(Request $request)
    {
        ...
        try {
            Excel::import(new EmployeeSalaryImports($company), $request->file('import_file'));

            // return redirect()->back()->with('success', 'Salaries imported successfully!');
            return Reply::successWithData(__('messages.SalariesimportedSuccessfully'), ['redirectUrl' => route('employee-salary.index')]);
        } catch (\Exception $e) {
    ...
    {
        $batch = $this->importJobProcess($request, EmployeesalaryImport::class, ImportEmployeesalaryJob::class);
