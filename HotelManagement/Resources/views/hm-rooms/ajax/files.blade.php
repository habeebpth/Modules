...
<div class="tab-pane fade show active mt-5" role="tabpanel" aria-labelledby="nav-email-tab">
    ...
    <x-cards.data :title="__('app.images')">
            ...
            <x-form id="save-taskfile-data-form" class="d-none">
                <input type="hidden" name="hm_room_id" value="{{ $hmrooms->id }}">
                <div class="row">
                    <div class="col-md-12">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0"
                                        aria-labelledby="dropdownMenuLink" tabindex="0">
                                           ...
                                           <a class="cursor-pointer d-block text-dark-grey f-13 py-3 px-3 "
                                                href="{{ route('hmroomfiles.download', md5($file->id)) }}">@lang('app.download')</a>
...
<script>
    $(document).ready(function () {
        ...
        taskDropzone.on('sending', function(file, xhr, formData) {
            var ids = "{{ $hmrooms->id }}";
            formData.append('hm_room_id', ids);
            $.easyBlockUI();
        });
    ...
    $('body').on('click', '.delete-file', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var url = "{{ route('hmroomfiles.destroy', ':id') }}";
